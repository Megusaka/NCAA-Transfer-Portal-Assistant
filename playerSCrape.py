...
#reponse content is the html content of the page that we want to parse and extract the player names 
...
#we get the selector from inspecting the html of the page and looking for the class name that contains the player names
# we can use the select method to get all the list items with that class
#li is a list item in html and the class is "sidearm-roster-player" which contains the player names
players = soup.select("li.sidearm-roster-player")
hometown = soup.select("span.sidearm-roster-player-hometown")
eligibility = soup.select("span.sidearm-roster-player-academic-year")
position = soup.select("span.sidearm-roster-player-position-long-short")
height = soup.select("span.sidearm-roster-player-height")


# we can then loop through the list of players and extract the name of each player
